...
export default function LogForm({ data, onAdd, onCancel, defaultAccount }) {
  ...
  const known = useMemo(
    () => data.entries.some((e) => (e.email || "").trim().toLowerCase() === f.email.trim().toLowerCase() && f.email.trim()),
    [data.entries, f.email]
  );
  const summary = useMemo(
    () => (known ? agentSummary(data.entries, f.email, f.date) : null),
    [known, data.entries, f.email, f.date]
  );

  const valid = Boolean(f.account && f.date && f.email.trim() && f.violation);
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 items-start">
      ...
      <div className="p-4 md:p-5" style={{ background: P.card, border: `1px solid ${P.line}`, borderRadius: 10 }}>
        ...
        <div className="flex gap-3 mt-4 justify-end">
          ...
          <BtnPrimary onClick={submit} disabled={!valid} title={valid ? "" : "Account, date, agent email and violation are required"}>
            Log entry
          </BtnPrimary>
        </div>
      </div>
